...
test_data = 'datasets/Houston/Houston.mat'
test_label = 'datasets/Houston/gt.mat'
...
class D_Res_3d_CNN(nn.Module):
    ...
    def forward(self, x): #x:(400,100,9,9)
        ...
        y1 = self.avgpool(x).view(x.shape[0],-1)          #torch.Size([16, 800])
        x = self.maxpool1(x) #(1,8,25,5,5)
        x = self.block2(x) #(1,16,25,5,5)
        y2 = self.avgpool(x).view(x.shape[0],-1)          #torch.Size([16, 400])
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(10000):  # EPISODE = 90000
        ...
        if episode % 2 == 0:
            ...
            # calculate features
            support_features, support_outputs,support_shallow_res1_features, support_shallow_res2_features = feature_encoder(supports.cuda())  # torch.Size([409, 32, 7, 3, 3])   ################################
            query_features, query_outputs,query_shallow_res1_features, query_shallow_res2_features = feature_encoder(querys.cuda())  # torch.Size([409, 32, 7, 3, 3])             #########################################
            target_features, target_outputs,target_shallow_res1_features, target_shallow_res2_features = feature_encoder(target_data.cuda(), domain='target')  # torch.Size([409, 32, 7, 3, 3])   ########################################
...
############################################################################################################
            ...
            # 使用排序索引对target_features进行重新排序
            sorted_target_features = target_features[target_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(target_label, return_counts=True)
            start_idx = 0
            target_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_target_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                target_features_proto.append(averaged_feature)
                start_idx = end_idx
...
############################################################################################################################################
            ...
            if SHOT_NUM_PER_CLASS > 1:
                support_shallow_res1_features_proto = support_shallow_res1_features.reshape(CLASS_NUM,
                                                                                            SHOT_NUM_PER_CLASS,
                                                                                            -1).mean(dim=1)  # (9, 160)
            else:
            ...
            target_sorted_indices = torch.argsort(target_label)
            # 使用排序索引对query_features进行重新排序
            sorted_target_shallow_res1_features = target_shallow_res1_features[target_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(target_label, return_counts=True)
            start_idx = 0
            target_shallow_res1_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_target_shallow_res1_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                target_shallow_res1_features_proto.append(averaged_feature)
                start_idx = end_idx
...
###############################################################################################################################################
 ...
 ##############################################################################################################################################
            ...
            # 求support_shallow_features的原型
            if SHOT_NUM_PER_CLASS > 1:
                support_shallow_res2_features_proto = support_shallow_res2_features.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS, -1).mean(dim=1)  # (9, 160)
            else:
            ...
            target_sorted_indices = torch.argsort(target_label)
            # 使用排序索引对query_features进行重新排序
            sorted_target_shallow_res2_features = target_shallow_res2_features[target_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(target_label, return_counts=True)
            start_idx = 0
            target_shallow_res2_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_target_shallow_res2_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                target_shallow_res2_features_proto.append(averaged_feature)
                start_idx = end_idx
           ...
           # randomlayer_out = random_layer.forward([shallow_features, softmax_output])     # torch.Size([225, 1024=32*7*3*3])
            ...
            shallow_res2_domain_logits = domain_classifier(randomlayer_out_shallow_res2, episode)
            shallow_res2_domain_loss = domain_criterion(shallow_res2_domain_logits, domain_label)
...
#########################################################################################################################
        ...
        # target domain few-shot + domain adaptation
            ...
            # calculate features
            support_features, support_outputs,support_shallow_res1_features, support_shallow_res2_features = feature_encoder(supports.cuda(),  domain='target')  # torch.Size([409, 32, 7, 3, 3])  ########################################
            query_features, query_outputs,query_shallow_res1_features, query_shallow_res2_features = feature_encoder(querys.cuda(), domain='target')  # torch.Size([409, 32, 7, 3, 3])    ########################################
            source_features, source_outputs,source_shallow_res1_features, source_shallow_res2_features = feature_encoder(source_data.cuda())  # torch.Size([409, 32, 7, 3, 3])     ########################################
            ...
            # 使用排序索引对query_features进行重新排序
            sorted_source_features = source_features[source_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(source_label, return_counts=True)
            start_idx = 0
            source_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_source_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                source_features_proto.append(averaged_feature)
                start_idx = end_idx
            ...
            # domain_label = torch.zeros([supports.shape[0] + querys.shape[0] + target_data.shape[0], 1]).cuda()
            # domain_label[:supports.shape[0] + querys.shape[0]] = 1  # torch.Size([225=9*20+9*4, 100, 9, 9])
            # print("support_proto_shape:",support_proto.shape[0])
            ...
            # 求support_shallow_features的原型
            if SHOT_NUM_PER_CLASS > 1:
                support_shallow_res1_features_proto = support_shallow_res1_features.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS,
                                                                                  -1).mean(dim=1)  # (9, 160)
            else:
                support_shallow_res1_features_proto = support_shallow_res1_features
            ...
            # 源域数据集生成排序索引
            source_sorted_indices = torch.argsort(source_label)
            # 使用排序索引对source_features进行重新排序
            sorted_source_shallow_res1_features =source_shallow_res1_features[source_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(source_label, return_counts=True)
            start_idx = 0
            source_shallow_res1_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_source_shallow_res1_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                source_shallow_res1_features_proto.append(averaged_feature)
                start_idx = end_idx
...
##############################################################################################################################################
            ...
            # 求support_shallow_features的原型
            if SHOT_NUM_PER_CLASS > 1:
                support_shallow_res2_features_proto = support_shallow_res2_features.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS,
                                                                                  -1).mean(dim=1)  # (9, 160)
            else:
                support_shallow_res2_features_proto = support_shallow_res2_features
            ...
            source_sorted_indices = torch.argsort(source_label)
            # 使用排序索引对source_features进行重新排序
            sorted_source_shallow_res2_features =source_shallow_res2_features[source_sorted_indices]
            # 按类别求平均值
            unique_labels, label_counts = torch.unique(source_label, return_counts=True)
            start_idx = 0
            source_shallow_res2_features_proto = []
            for count in label_counts:
                end_idx = start_idx + count
                class_features = sorted_source_shallow_res2_features[start_idx:end_idx]
                averaged_feature = torch.mean(class_features, dim=0)
                source_shallow_res2_features_proto.append(averaged_feature)
                start_idx = end_idx
...
##########################################################################################################################
        ...
        if (episode + 1) % 1000 == 0 or episode == 0:
            ...
            train_datas, train_labels = train_loader.__iter__().next()
            train_features, _ ,domain_res1,domain_res2 = feature_encoder(Variable(train_datas).cuda(), domain='target')  # (45, 160)  #####################
            ...
            for test_datas, test_labels in test_loader:
                ...
                test_features, _ ,domain_res1,domain_res2= feature_encoder(Variable(test_datas).cuda(), domain='target')  # (100, 160) ########################
            ...
            if test_accuracy > last_accuracy:
                # save networks
                torch.save(feature_encoder.state_dict(),str( "checkpoints/DFSL_feature_encoder_" + "Houston_" +str(iDataSet) +"iter_" + str(TEST_LSAMPLE_NUM_PER_CLASS) +"shot.pkl"))
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,  "classificationMap/Houston_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
